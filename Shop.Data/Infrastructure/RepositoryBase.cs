...
namespace Shop.Data.Infrastructure
{
    public abstract class RepositoryBase<T> : IRepository<T> where T : class
    {
        ...
        public IQueryable<T> GetAll(string[] inclules = null)
        {
            //HANDLE INCLUDES FOR ASSOCIATED OBJECTS IF APPLICABLE
            if (inclules != null && inclules.Count() > 0)
            {
                var query = dataContext.Set<T>().Include(inclules.First());
                foreach (var include in inclules.Skip(1))
                {
